...
Onlyne 现在是单 crate 的工作区级 IM daemon（`onlyne` 0.6.0，`src/` 7688 行，`src/adapters/` 里硬编码四平台的 adapter 工厂）。多 agent 编排不在本仓，全在仓外的独立 crate `harness/onlyne-swarm`（0.7.0，约 14k 行）。两者靠 `loopback` channel 耦合，swarm 元数据夹带在 `text` 里，走 `---swarm` 正文头协议（`harness/onlyne-swarm/PROTOCOL.md:1-6`）。v1.0.0 把它重构成**高内聚、低集成、可跨机部署、可递归组网**的 agent 通信组件。分工：`server` 做投递路由 + 账本 + 协议转换 + gateway 宿主；`client` 是每工作区一个的 role 运行时，管 session 生命周期、进程 backend、intent；coding-agent 插件与外部 IM gateway 实现同一份 adapter 协议，分别挂在不同侧。人→IM→agent 变成"gateway 投递给 role"的特例。零兼容：旧配置、旧 DB、旧 wire 一律 fail-fast，迁移由 agent 手工完成。

终态判据全在 **Verification** 一节，都是可直接跑的命令。
...
依赖规则（违反即返工）：`onlyne-proto` 不依赖 tokio。`onlyne-session` 只有纯函数和 `SessionBackend` trait，不依赖 `onlyne-store`。`onlyne-server` 与 `onlyne-client` 互不依赖，只共享 `proto/frame/net/store/config/layout`。`plugins/*` 只依赖 `onlyne-adapter` + `onlyne-proto`，永不依赖 server 内部 crate。

二进制边界（D17）：server 不编译任何平台 SDK（`teloxide`/`openlark`/`wechat-ilink`/`tokio-tungstenite`），也不编译 `resvg`/`pulldown-cmark`。gateway 二进制不编译 ledger、router 与 TLS 服务端。`onlyne-client` 不编译任何平台 SDK。规模可扩展性由此保证：一台只跑 role 的机器只需要 `onlyne-client`。
...
发现规则沿用向上走查（`src/workspace.rs:18-26`）。但 `Workspace::bootstrap` 一旦看到旧布局标志就拒绝：存在 `.onlyne/state.db` 且其中含表 `io_cursors` 或 `loopback_idempotency`，或存在 `.onlyne/channels/`。此时打印 `onlyne: legacy workspace layout; v1.0.0 does not migrate`，并 exit code 2。

### 3. 统一消息格式（`crates/onlyne-proto`）

今天有两处耦合要替换。一是 `MessageEnvelope.channel_id: String`：它要么是 adapter key，要么是字面量 `"loopback"`（`src/core.rs:86-101`、`src/app.rs:354-396`）。二是 swarm 元数据：它夹带在 `text` 里，走 `---swarm` 头（`harness/onlyne-swarm/PROTOCOL.md:9-36`）。两处全部删除。
...
`op_id` 只有一种写法，各端一致：`"o-" + uuid_v4(发送方进程启动时生成)`。重试沿用同一 `op_id`。幂等指纹 = `sha256_hex(serde_json::to_vec(&Envelope 去掉 id/ts/op_id))`。冲突文案固定为 `"op_id conflict: request differs from durable receipt"`（沿用 `src/store.rs:216-217` 的语义与措辞）。

`kind` 语义：
- `Task` —— 投给某个 role，要求它产生或复用一个 session。根提交与下游转派只差在 `causality.parent_task`。投递这个动作本身就创建任务，没有中央 dispatch（D12）。
- `Completion` —— 某个 task 的终态回执。`causality.task` = 被完成的任务，`body.text` = 结果摘要（取前 200 字进 ledger 的 `out_head`，沿用 `PROTOCOL.md:51-52`）。`outcome` 放在 `ReportKind::Complete` 的 `outcome` 字段，见 §6。
- `Note` —— 自由文本。不建 session，不进队列；目标离线就直接 `recipient_offline` 拒收。人→agent 的普通聊天和 agent 之间的闲聊都走它。
- `Control` —— `recycle/probe/snapshot/cancel`。只有 `admin = true`，或发起方就是该 task 的属主 role，才放行。

### 4. 帧协议与连接

`crates/onlyne-frame`（没有现成等价物：今天只有 `BufReader::lines()` 的 NDJSON，`src/ipc.rs:116,245-250`）：
...
一个帧 = `u32 big-endian 长度` + 这么长的 JSON。超过 `MAX_FRAME_BYTES` 就发 `error{code:"frame_too_large"}`，然后关连接。
...
`res.error.code` 取值是封闭集，全部小写下划线：`invalid`、`unknown_op`、`acl_denied`、`unknown_role`、`recipient_offline`、`duplicate`、`conflict`、`unauthorized`、`forbidden`、`not_admin`、`frame_too_large`、`bad_frame`、`protocol_version`、`internal`。今天的现状只有 `error` / `bad_json` 两个码（`src/ipc.rs:134,220,236`），本次一并替换。

观测面（D11）：`ev` 不自带重放。订阅体是 `{op:"subscribe", since_seq, tiers, kinds}`。10s 收不到 `ev` 就由客户端发 `ping`，`pong` 带回 `server_seq`；落后超过 `resync_lag`（默认 256）就重新 `subscribe` 并带 `since_seq`。事件的 `seq` 单调递增，服务端重启后从 ledger 的 `events` 表恢复计数起点。
...
ACL 判定放在 `onlyne-net`：`pub fn acl_allows(spec:&Spec, from:&Principal, to:&Principal, kind:MsgKind) -> Result<(), AclDeny>`，在 ledger 落盘**之前**调用。`aggregate` role 对上层只出现在它自己所属 server 的 spec 里；核心投递路径没有 aggregate 分支（D14）。

### 6. client：session 生命周期与进程 backend

下面这些实现已经验证过，直接搬，不要重写：

- `harness/onlyne-swarm/src/lifecycle.rs`（1447 行：`AgentState{Booting,Ready,Running,Idle,Gone}` × `DeliveryState{None,Pending,Retrying,Accepted,Exhausted}` × `ResourceState{Detached,Attached,Closing,Closed}` × `RecoveryState{None,IdleWaiting,IdleFault,Draining}` × `Outcome` → `PublicLifecycle{Created,Working,Idle,Exited}`；21 个 `LifecycleEvent` 变体各带 `Version{generation,seq}`；`apply()` 全函数 + `is_legal()` + 表驱动测试 886-1447）原样进 `crates/onlyne-session/src/lifecycle.rs`，只把 `use crate::db` 这类引用改成适配本地 trait。测试随行迁移，不许减少断言。
- `harness/onlyne-swarm/src/runtime/mod.rs` 的 `SessionBackend` trait / `Capabilities{spawn,attach,probe,close,focus,rename}` / `SpawnSpec{cwd,task_id,command,env,focus,rename}` / `SessionRef{task_id,backend,backend_ref:Value,generation}` / `ResourceProbe{alive,attached,detail}` / `CloseReason{Completed,Cancelled,Fault,Shutdown,Replaced,Operator}` → `crates/onlyne-session/src/backend/mod.rs`。`orca.rs`、`zellij.rs`、`fake.rs` 平移；`herdr.rs` 删除（它是个文档桩，所有方法都返回 unsupported）。`select_backend` 的探测序改成 `zellij → orca → fake`，环境变量改名 `ONLYNE_BACKEND`。
- `harness/onlyne-swarm/src/reconcile.rs` 里"reducer ↔ 持久 ledger"的桥接（`apply_persist`、`seed_created`、`stored_observation`、`to_versioned`、`backend_ref_json`，行 90-310）→ `crates/onlyne-session/src/reconcile.rs`。其中的自动策略全删：recovery task 生成、`sweep_dead_terminals` 的重投、`hop_timeouts` 触发 replay、`MAX_ATTEMPTS` 到限自动重投。保留事实归约、`(generation,seq)` 单调写入门禁、fault 记录与事件外发（D15/D16 的 timeout_policy 决定：core 只检测、只记 fault）。
- `harness/onlyne-swarm/src/sched.rs` 的 `on_ready`/`dispatch`/`session_alive`/`on_out`/`on_recycled` 编排骨架 → `crates/onlyne-client/src/dispatch.rs`。身份匹配键不再用 `terminal_handle`，改走 env 传递（`ONLYNE_SESSION_ID`、`ONLYNE_TASK_ID`）。同时删掉 `sched.terminals: HashMap<String,String>` 和 `replay_ready_history` 里按 handle 匹配的逻辑——跨机不成立（见 `src/sched.rs:334-404`、`src/events.rs:79-161`）。

client 收到 Task 后的机械流程：先查 `reuse` 策略。有 idle 且未绑定 task 的 session 就复用；否则 `backend.spawn(SpawnSpec{command: spec.session_command 渲染 {session}/{task}, env 注入 §7, cwd = workspace})`。然后记录 `sessions` 行。接着向 server 上报 `report{kind:"ready"}`，之后才 `deliver` 载荷——保留今天 ready barrier 的因果顺序：先 ready，后投递（`src/sched.rs:334-404`）。

出向 intent（D3/D11）存在 `client.db` 的 `intents(op_id PK, env_json, attempt, state ∈ {pending,retrying,accepted,exhausted}, next_attempt_at, receipt_json, last_error)`。指数退避读 spec 的 `intent.backoff_ms`，次数读 `intent.attempts`。耗尽就本地记 `fault{kind:"intent_exhausted"}`，同时上报 `report{kind:"fault"}`，绝不静默丢弃。

断连行为（D3）：连接失败或收到 `bye` 后，停止接受 server 的新投递（本地 `accept_new = false`）。在跑的 session 继续跑到终态；终态产生的 completion 走 intent 落盘重试。`accept_new=false` 之后不再 spawn 新 session。client 退避重连（1/2/4/8/…/60s 封顶），重连成功后按 `seq` 顺序 flush intents。

### 7. adapter 协议（D16，一份协议两侧挂载）

两侧共用同一套 `onlyne-adapter` SDK 与帧编解码。agent 插件连 `<workspace>/.onlyne/run/s`，gateway 进程连 `<server-root>/.onlyne/run/s`。admin socket 与 adapter socket 复用同一个监听，握手时按 `hello.kind` 分流：`agent` / `gateway` / `admin`。
...
宿主 → 插件：`welcome{role, prose, session_id, generation, server:{connected,cluster,name}}`、`assign{envelope, prose}`（agent 侧）、`render_send{envelope}`（gateway 侧，平台格式转换由插件做）、`probe{}`、`recycle{task_id,reason}`、`config_get{key}`、`bye{reason}`。

能力协商：`capabilities` 里没有 `recycle`，宿主就只能靠 `probe` 判定资源消失。没有 `report`，该 session 走 `idle_fault` 并记 fault。没有 `assign`（纯 CLI 型 agent），载荷改用 stdin/参数交付，终态看进程退出码加末行输出。`hello` 超时 5s；还没 `hello` 就发别的帧，回 `error{code:"invalid",message:"hello required first"}` 并断连。

agent 侧 SDK 至少要覆盖这些抽象，清单来自两个现有插件的实测差异（pi-onlyne vs dsh-onlyne）：`configPath`、`registerTool`、`registerCommand`、`wakeUser`（pi `pi.sendUserMessage(...,{deliverAs:"followUp"})` / dsh `agent.followup(createUserMessage(...))`）、`sendCustomEntry`、`on(turn lifecycle hooks)`、`exit(reason)`、`wrapResult`、`setActiveTools`、`setStatus/setTitle`、`setModel/setThinkingLevel`。SDK 把每一项都声明为可选 trait 成员；宿主缺哪个就报对应能力缺失。

`onlyne-adapter` 另外发布一份机器可读的协议规范：`crates/onlyne-adapter/PROTOCOL.md` 与 `onlyne-adapter.schema.json`（后者由 `onlyne-proto` 的 schemars 导出）。外置 TS 插件仓照它对齐（D18）。
...
今天残留的 op 全部删除：`loopback`、`swarm_ready`、`swarm_recycled`、`swarm_busy`、`swarm_idle`、`mark_io_consumed`、`consume`、`start_adapter`、`stop_adapter`、`restart_adapter`、`fetch_history_page` 的旧形态、`onlyne client '<json>'` 原样透传（`src/app.rs:166-232`、`src/cli.rs:119`）。唤醒自己 = 向自身 role 发 `note`。
...
准入登记流程（配合 D13 的文件唯一真相）：`onlyne-client init --workspace W --role R --server-root S` 会生成 `W/.onlyne/keys/role.key`（ed25519），经 S 的 admin socket 读 `[server]` 段写入 `W/.onlyne/config.toml`，并向 stdout 打印一段可直接粘贴的 spec 片段（首行恰为 `[[client]]`，含 `role` 与 `key = "ed25519/<base64>"`）。`init` 自己绝不写 spec 文件。操作员/supervisor 把片段追加进 `spec.toml`，再执行 `onlyne reload` 生效；未登记的 key 连接时会得到 `error{code:"unauthorized"}`。
...
**真相分工**：`spec.toml` 管协议真相——role 名、公钥、ACL、prose、并发、超时、`session_command`。`<server-root>/.onlyne/templates/<相对路径>/` 管内容真相——role 工作区里除运行期件以外的一切（`AGENTS.md`、`prompts/*.md`、`.pi/settings.json`、`.pi/onlyne.json` 等）。模板内容对 onlyne 不透明，只有一个例外：模板里可以放一个 `.onlyne/config.toml` 作为本地覆盖片段，与 §5 的派生值合并，派生值优先。
...
- `--out` 默认 `<server-root>/.onlyne/ws`。默认遍历 `spec.toml` 里全部 `[[client]]` 条目，每条生成一份工作区，带 `aggregate` 的 supervisor role 条目也算——它在本集群里同样是本地 role。`--template` / `--role` 用来取子集；两个同时给出时取交集，交集为空就报 `onlyne: no role matches the requested templates/roles`，退出码 4，不写任何文件。
...
- 目标已存在又没给 `--force`，就报 `onlyne: refusing to overwrite <path>; pass --force`，退出码 4，不写任何文件。`--force` 只覆盖 `.onlyne/config.toml`、`.onlyne/keys/role.key`（仅当不存在时生成）与模板内容文件；已存在的 `.onlyne/client.db`、`.onlyne/run/`、`.onlyne/logs/` **永不覆盖**。
...
**可搬迁硬约束**：产物里任何文件都不许含生成时的绝对路径。做法是写完扫一遍全部产物的字节，找 `out.canonicalize()` 与 `<server-root>` 两个前缀字符串；命中就删掉本次输出目录，报 `onlyne: generated workspace embeds absolute path <path>`，退出码 4。运行期路径全部由 client 从自己的 `--workspace` 推导（`.onlyne/run/s`、`client.db`、`logs/`），连 server 靠 `listen` + `cert_pin`。因此目录搬到任何路径、任何机器，直接 `onlyne client run` 就能跑。
...
按这个顺序执行。每步结束都要 `cargo build` 通过、该步新增测试通过，且已迁入库仍可编译。步骤 3 之后，各库之间只经 `onlyne-proto` 通信，可以并行推进（标了 `[可并行]` 的步骤）。

### S1. 拆除旧耦合，立起 workspace 骨架

1. 根 `Cargo.toml` 转 `[workspace]`，把现 `src/` 整体移入 `crates/onlyne-legacy/src/`（临时包名 `onlyne-legacy`，只在搬迁期引用，S12 删除）。再新增 §1 列出的空 crate，每个只放 `lib.rs` + 一个 smoke 测试。
2. 删掉 `harness/onlyne-swarm`、`harness/pi-onlyne`、`harness/dsh-onlyne` 三个 submodule（`git rm <path>` + 删 `.gitmodules` 对应段）。swarm 源码用 `git -C harness/onlyne-swarm show 1a2aefd:<file>` 落地到 `crates/onlyne-session/`（S3 用到时逐文件取）。删之前要先确认已把 `lifecycle.rs`、`runtime/{mod,orca,zellij,fake}.rs`、`reconcile.rs`、`sched.rs`、`db.rs`、`proto.rs` 复制入树。注意 `harness/pi-onlyne` 当前 checkout 在 `origin/dev` 72e5fa6（父仓 main 记录的 gitlink `75c6b0d` 在其远端不可达），只作协议参考，不再搬迁代码。
3. `build.rs` 改成 workspace 级：schema 生成目标迁到 `crates/onlyne-proto/build.rs`。
4. 验证：`cargo build --workspace` 通过。

### S2. 帧与协议

1. 写 `onlyne-frame`：§4 的 `write_frame`/`read_frame`（`tokio::io::AsyncReadExt::read_u32_be`），覆盖超限分支，并测半包/粘包。核心没有现成等价物（今天只有 `ipc.rs:116` 的行分隔读法）。
...
3. 表驱动测试覆盖三件事：`Envelope` 往返、每个错误码都可达、`validate()` 对超限 text/image 的拒绝文案逐字断言。

### S3. session 内核搬迁

搬迁 §6 的三项（lifecycle / backend / reconcile 桥接）。删除清单：`runtime/herdr.rs`、`reconcile.rs` 内自动 recovery task 生成、`sched.rs::sweep_dead_terminals` 的自动重投分支、`hop_timeouts` 中触发 replay 的分支（记录 fault 的分支保留）、`events.rs::replay_ready_history`（terminal_handle 匹配跨机不成立）。`session_alive` 只走 `backend.probe` 一条路径；删掉把 `handle` 以 `stub-` 前缀视为存活的旁路（`src/sched.rs:594-611`）。验证：lifecycle 全表测试原样通过；`fake` backend 下断言 `spawn→probe→close` 的状态推进。

### S4. 配置、布局、store

`onlyne-config` 放三个结构体（server spec、client config、plugin config），全部 TOML；沿用 `src/config.rs` 的 `Env::secret`（env 间接取值）与 `$VAR` 语法。`onlyne-layout` 去掉 channels，加 legacy 检测，命中就 exit 2。`onlyne-store` 按 §10 做两个模块，加 `schema_marker` 门禁与 `(generation,seq)` 单调 upsert，写法照抄 `harness/onlyne-swarm/src/db.rs:351-364`。验证：`sqlite` 打开旧布局样本 DB 要拒绝；spec 缺字段或出现未知字段要报错并带行号。

### S5. 传输与准入 `[可并行]`

`onlyne-net` 提供：TLS acceptor（自签 + `cert_pin` 指纹核对）、`hello` 挑战签名（server 发 32 随机字节，client/gateway 用私钥签，server 用 spec 里登记的公钥验）、`acl_allows`、`backoff` 工具（1/2/4/8/…/60s）。为什么新写：核心今天零鉴权（`src/ipc.rs:70-249`，`shutdown` 任何本地进程都能调），没有 TLS/身份实现可复用。测试：错 key 拒、未登记公钥拒、指纹不符拒；`allowed_senders`/`allowed_targets` 命中和不命中各一例。

### S6. server 运行时

`router.rs`（§8 的 op match）、`relay.rs`（`send` → ACL → `ledger` 落盘 → `pull` 投递 → `ack` 结算；目标离线时保留 `queued`，`note` 是否丢弃看 `note_queue`）、`projection.rs`（`session_sync`/`report` 落镜像与 events）、`faults.rs`（只记录 + 外发，不自动修复）、`admin.rs`（admin socket：只读查询 + `repair_*` 事务性 ledger 编辑 + `watch` 流 + `reload` + `spec_diff`）。验证：两个 client 加一个 role 做三角投递，`onlyne server ledger` 输出状态序列 `queued→in_flight→acked`；同一 `op_id` 重发得到 `duplicate`，并返回原 receipt。

### S7. role 工作区生成与放置

实现 §11：`crates/onlyne-config/src/template.rs`（层级走查 + 深合并 + 占位符替换 + 绝对路径扫描）、`crates/onlyne-server/src/generate.rs`（`generate` op 与 CLI 动词，写出 `<out>/<层级>/<role>/` + `[[client]]` 片段 + `.onlyne-generation.json`）。测试点：两个模板两个 role 的产出树；没设 `agent_package` 而模板用到要逐字报错；产物含绝对路径要清理并退出码 4；`--force` 不碰 `client.db`；搬到新路径后 `client run` 不改任何东西就能连上（用 §2 的 config 解析路径断言）。

### S8. client 运行时 `[可并行]`

`runloop.rs`（server 连接、intents flush、断连按 D3 行为）、`adapter_socket.rs`（按 `hello` 分流 agent/admin、处理 §7 的 op、能力缺失记 fault）、`local_cli.rs`（`send/reply/complete/handoff/control` 的本机入口，落 intent）、`accept.rs`（ready barrier → 复用/spawn → assign 插件 → `ack`）。验证：在 `fake` backend + `onlyne-agent-fake --workspace <ws> --script ...` 下，一条 `Task` 从 server 抵达 → 插件收到 `assign` → `Completion` 回 ledger 变 `acked`。
...
2. `src/adapters/{telegram,feishu,qqbot,weixin}.rs` → `plugins/onlyne-gateway-<platform>/src/lib.rs`。每个都实现 `onlyne-adapter::GatewayPlugin`：入站把平台事件转成 `Envelope{kind:Note|Task, from:Principal::Gateway{...}}`，走 `deliver`；出站从 `render_send` 取 `Envelope`，做平台渲染（markdown→card/html/text/表格转 PNG 作 `Body.image`）后调平台 API。平台细节只存 `reply_to`/`causality` 所需的最小关联，不再塞进 `platform_metadata: Value`（今天四个 adapter 都往里塞自由 JSON，`src/core.rs:100`）。
   每个平台 crate 由 `onlyne-gateway` 的 Cargo feature 门控（`telegram` / `feishu` / `qqbot` / `weixin`，default = 全部），`--platform` 负责运行期选择。单平台部署可以 `--no-default-features --features telegram` 只链一家，避免平台 SDK 进到不需要的进程。四家 SDK 永不进入 `onlyne-server` / `onlyne-client`。
3. 搬迁时一并清掉已核实的空壳与泄漏，不留旧行为：
   - `Adapter::list_conversations` 四个实现全是 `Ok(vec![])`（`src/adapters/telegram.rs:182-184`、`feishu.rs:126-128`、`qqbot.rs:217-219`、`weixin.rs:183-185`），在 adapter 协议里降级为可选能力。四平台拿不到真实会话列表就声明没有该能力，`conversations` 表不再作为副作用填充。
   - `Event::DeliveryUpdate`（`src/core.rs:129-134`）与 `Event::Error`（`src/core.rs:160-163`）只出现在枚举定义和 `src/ipc.rs:256,264` 的字符串映射里，全仓没有 publisher；`AdapterHealth::Starting`（`src/core.rs:48-56`）从未被构造。这三者都从 `onlyne-proto` 删除：`DeliveryUpdate` 的语义交给 ledger 状态事件，`Error` 交给 `fault`。
   - `start_adapter` / `stop_adapter` / `restart_adapter` 是空壳，只返回 `{"started": false}` + `Event::Warning`（`src/app.rs:220-230`），一并删除。gateway 进程的起停交给 supervisor，用 CLI/shell 负责。
   - 平台原始负载不再进统一信封。今天的做法：`feishu.rs:544` 存整包 raw payload、`weixin.rs:415` 存 `to_value(&msg.raw)`、`qqbot.rs:748,804` 靠 `platform_metadata` 里的 `/onlyne/qq_scene` 指针选场景、`telegram.rs:320` 塞 chat title/username。改成 gateway 本地关联表 `gateway_ref(channel, conversation, external_id, scene)`，存在 gateway 自己的 DB 里；跨进程只传 `Principal::Gateway` 与 `reply_to`。
4. 验证：除 `FakeGateway` 外，真实跑 `onlyne gateway run telegram`、不带凭据时，报错文案要含缺失的 env 名；server 记 `fault{kind:"gateway_unconfigured"}`，而不是 panic。

### S11. 递归集群路径（零新代码的验证性工作）

按 D14/D15，真正要写的代码只有一处：supervisor 的 client 用「父 spec 中登记的 aggregate role 公钥」连父 server，`hello.args.mount.role` 填 aggregate 名。本步骤交付：
1. `onlyne-cli` 增加 `onlyne cluster export-prose`：打印本 role 的对外说明文本，供上层写进它自己的 prose。这样就把子 cluster 的接口信息交给上层 supervisor。
2. 文档性产物 `crates/onlyne-server/FEDERATION.md`（只是协议约定，不是运行时代码）：aggregate role 在父 spec 里就是一个 `[[client]]` 条目；`allowed_targets` 只含父层可见的 role；子层内部拓扑永不出现在父 ledger。
3. 验证见 Verification 的「两 cluster 联邦」用例：父层 ledger 里对 aggregate 的投递只有父层可见；父 ledger 的 `body_json` 不含任何子层 role 名；父层 completion 的 `out_head` 也不含子层 role 名。

### S12. 删除遗留与收口

删 `crates/onlyne-legacy/`、`src/` 全部旧文件、`docs/IPC.md`、`docs/CHANNEL_IO.md`、`docs/RICH_MEDIA.md`（FIFO/富媒体已不存在）、`onlyne-config.schema.json`（根，改由各 schema 生成物取代）、`examples/` 下 `fifo/`、`broadcast/`、`multicast/`、`multi-channel/`、`rich-media/`（这些只是 `send_message` 的 CLI 循环，没有协议内容，`examples/shared/send-many.py:22-33`），以及依赖旧 op 的 `scripts/pi-onlyne-longhaul.mjs`。`.agents/skills/onlyne/SKILL.md` 与 `skills/onlyne-channel-smoke/SKILL.md` 按新 CLI 词表重写成一份。`README.md` / `README.zh-CN.md` 按三进程模型重写。`AGENTS.md` 的 §0（"Onlyne is not a workflow engine / agent runtime"）、§5（工作区布局）、§6（IPC 最小 op 集）、§11（"no plugin systems"）、§12（交付策略）改写成 v1.0.0 的边界：server 只投递不编排、编排归 role 与 supervisor、adapter 两侧挂载、零兼容。
...
1. 单机端到端（新行为主证）：脚本 `crates/onlyne-testkit/e2e/local-task.sh`，用 `set -euo pipefail`，前置 `cargo build --workspace`：
   ```
   ...
   ```
   期望：`init` 输出的首行恰为 `[[client]]` 且含 `key = "ed25519/`，要在 `wait-ready` 之前；`send` 输出一行 JSON，`ok = true`、`data.task` 是 uuid v4、`data.state = "in_flight"`；随后 `onlyne --server-root "$tmp/server" ledger --task <task>` 的 `state` 最终为 `acked`、`out_head` 含 `hello v1`；`onlyne ... sessions --task <task>` 的 `public_lifecycle = "exited"` 且 `outcome = "done"`；`echo-complete.json` 里断言 fake agent 收到的 `assign.prose` 等于 spec 的 `prose`。
2. ACL 硬拒（准入模型的回归，必须从真实 client 发出）：在 spec 里给 `builder` 设 `allowed_targets = ["planner"]`，按用例 1 的方式登记，并起 builder 的 client 与 fake agent，然后
   `onlyne --workspace "$tmp/builder" send --to reviewer --text x`
   → 期望 `ok = false`、`error.code = "acl_denied"`、`error.field = "to.role"`；server 的 `ledger` 没有新行；builder 的 `intents` 表无残留行（发送侧收到拒答后不落 intent）。
3. 幂等与重放（控制面 at-least-once 的硬证）：先按 §3 的规范手工固定一个 `op_id`，对同一 role 连发两次内容相同的 `send` → 第二次期望 `ok = false`、`error.code = "duplicate"`，且 `data` 逐字等于首次的 receipt（含同一 `msg_id`）。再改 `body.text` 复用同一 `op_id` 重发 → 期望 `error.code = "conflict"`、`error.message` 逐字等于 `op_id conflict: request differs from durable receipt`。两次请求在 `ledger` 合计只有一行。
4. 断连与恢复：起 server + client，投 3 条 Task，然后 `kill -9` client。server 日志或 `roles` 查询要出现 `state="offline"`，`queued` 行保留。client 重跑后 3 条按 `seq` 顺序补投，ledger 三条各 `acked` 一次，`sessions` 表 task 行数恰好为 3（没有重复 session）。在跑 session 的 completion 通过 intent 落 `pending`，重连后送达。
...
- server 的 role 条目与 client 一一对应，一个 client 只服务一个 role。若实现时发现某个 role 需要多个 client 实例承载，回退做法是在同 role 条目下加 `replicas = N`，轮询选实例；寻址格式不变。
- `note_queue` 默认关闭（note 不排队，目标离线就直接 `recipient_offline`）。这是 §5 里唯一带产品倾向的默认值。
