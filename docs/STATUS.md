...
Counts come from `cargo test --workspace` on 2026-09-14 (692 passed, 0 failed), one line per crate with its libraries and integration targets summed.
...
- [x] `onlyne-server` green with router, relay, projection, faults, admin, and generate: 8 unit + 67 delivery + 27 generate + 2 stale.
