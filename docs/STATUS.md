...
Counts come from `cargo test --workspace` on 2026-09-15 (709 passed, 0 failed), one line per crate with its libraries and integration targets summed.
...
- [x] `onlyne-config` green with spec parse and reload: 11 template + 23 config contract + 17 ACL table + 3 spec example.
- [x] `onlyne-layout` green with legacy refusal exit 2: 15.
- [x] `onlyne-store` green with ledger and local DB: 28 unit + 2 schema statements.
...
- [x] `onlyne-server` green with router, relay, projection, faults, admin, and generate: 13 unit + 73 delivery + 27 generate + 2 stale.
- [x] `onlyne-client` green with runloop, intents, adapter socket, host detection, and dispatch: 24 unit + 41 scenarios.
- [x] `onlyne-tui` green with the role network graph, the swarm monitor, and the key table: 70.
