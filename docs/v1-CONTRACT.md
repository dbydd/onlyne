...
- `onlyne-net` does not depend on `onlyne-config`. It owns `RoleAcl`, `AclTable`, `MsgClass`, and `acl_allows(table, from, to, class, owner: Option<&str>) -> Result<(), AclDeny>`, keyed by role-name strings. The server maps `spec.toml` rows into those types and translates `AclDeny { reason, field }` into `ErrorCode`. `docs/v1-PLAN.md` §5 line 274 writes the signature as `acl_allows(spec, from, to, kind)`; the shipped shape is the deviation, recorded here, and the decision function still lives in `onlyne-net`.
