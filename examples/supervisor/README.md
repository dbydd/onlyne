...
整套东西只依赖 Onlyne 自己的产品面：spec 里的 prose、`generate` 出来的工作区、
`client start` 拉起的会话、`send` 派活、ledger 收尾。没有为演示发明的新机制。

集群里有一个操作 agent `_supervisor` 和一条五元环 `a,b,c,d,e`。`_supervisor`
是用户的集群操作 agent：它的 pi 进程跑在用户自己的终端里（在 Orca 里就是一个
标签页），用 `onlyne` CLI 派活、看账、开关集群。环上的五个角色由服务器管理：
每个任务由客户端拉起一个 pi 会话（在 Orca 里各占一个标签页），会话跑完就退出，
标签页随之关闭。
...
六个角色的工作区、记录文件 `lights.txt` 都在那里。放在短路径下是因为 macOS 的
unix socket 路径上限 104 字节——worktree 本身已经深了 93 字节，运行时再进仓库
就会撞 `bind: Unsupported socket address`。仓库里只留源（模板与脚本）。

## 模型（D15）

- **worker 角色的生命周期归服务器**：`_supervisor` 之外的每个 `[[client]]` 条目
  都有客户端，任务到了就拉起会话，会话终态由插件回执。
- **supervisor 的进程归用户和终端**：它是普通的 pi 会话，用户在标签页里跟它说
  话。服务器从不 spawn、recycle 或 probe 它，它的 `[[client]]` 条目只做两件事：
  登记操作者身份（`admin = true`，admin socket 的 `send` 需要 `--from` 已登记且
  带队 admin 位），以及给出 ACL 锚点。它的 key 由 `generate` 一并产出。
- 环上每个成员只认识自己的前后邻居：`allowed_targets` 放下行边和回执边，
  `allowed_senders` 是配对的一半，`Spec::acl_edges` 只在两边都认这条边时才发射它。
...
3. `onlyne generate --out /tmp/onlyne-sup/ws` 生成
   ...
   是脚本在照 e2e case 9 的先例替操作者做；真实部署里这是人的决定。
4. `onlyne server start` + `client start` ×5（每个角色一个后台进程，日志在
   `<ws>/.onlyne/logs/`）。
5. 等环上五个角色上线，把 `_supervisor` 的 pi 作为标签页打开（`orca terminal
   create`），第一句话就是 `up` 的参数。**脚本不等环跑完**：派活由 supervisor 自己
   发出，`up` 打完提示就退出。

每个会话标签页里跑的是完整的交互式 pi TUI（`session_command` 里没有
`--mode rpc`）。任务文本不走 argv：`{task}` 在客户端渲染成任务 **id**，正文由
pi-onlyne 插件在 `assign` 时注入，所以 TUI 里能直接看见模型读任务、干活、交差。

## 一轮流水灯

环是 `RING=a,b,c,d,e`，跑两圈共 `5 × 2 = 10` 跳。任务文本只有四个字段：

```text
RING=a,b,c,d,e FILE=/tmp/onlyne-sup/lights.txt K=1 TOTAL=10
```

`a` 收到 K=1：追加 `1:a` 到记录文件，然后用 `onlyne handoff` 把同样的文本交给 `b`，
K 加一；`b` 追加 `2:b` 交给 `c`……`e` 在 K=10 时不再转发，读文件、用
`onlyne_complete` 把文件内容交回。账本里每个任务一行（根任务加九个 handoff 子任务），
每个子任务都记着自己的 `parent_task` 和 `hop`。`handoff` 走产品自己的路径：它回读
父任务行，把新任务挂在 `parent_task` 下，`hop` 取父行加一。

整条链是 `len(RING) * 2` 次冷启动的 pi 加十轮模型回合，跑完是分钟量级。`run.py`
的默认任务是让 supervisor 去开这一轮；记录文件长到 `TOTAL` 行就是灯全亮。

## 实地观看

这套演示的看点就是它真的在动。在 Orca 标签页里跑 `run.py up`：

1. **supervisor 标签页**：标题是 `onlyne-supervisor`，里面是交互式 pi，cwd 是
   `ws/demo/_supervisor`，所以它的 `AGENTS.md` 与集群路径都加载好了。它先用
...
2. **环上的会话标签页**：每跳一个 pi 会话，`a → b → c → d → e → a → …` 依此亮起，
   标签页里是完整的 pi TUI（初始会话，正文由适配器投进来），能看见模型读任务、
   `echo` 追加一行、再 `handoff` 给下一个字母；会话完成时 pi 自己退出，Orca 随之
   回收标签页。十跳就是十次亮灭，这是演示的视觉主体。`lights.txt` 实时变长，
   `onlyne --server-root /tmp/onlyne-sup sessions --json` 的每一行都带
   `observed.host.orca.pane_key`，即这条会话绑定的真实 pane：
   `pane_key = "<tabId>:<leafId>"`。
3. **TUI**：另开一个标签页，`target/debug/onlyne-tui --server-root /tmp/onlyne-sup`。
   第 1 页是角色网络（五个环成员在线，`_supervisor` 显示离线——离线在这里的含义是
...
4. **收摊**：`run.py stop` 关掉它开的 supervisor 标签页（关完会再查一遍，没关掉就
   报错），清掉还留在 Orca 里的会话标签页，再停五个客户端与服务器。

两种会话载体是同一个协议的两种摆法。默认（Orca 机器上）是 TUI：标签页是给人看的，
初始会话由适配器注入任务正文。无头场合（CI、没有 Orca）用 `export
ONLYNE_BACKEND=exec`，会话变成客户端自己的子进程，supervisor 则作为子进程把输出写进
`/tmp/onlyne-sup/logs/supervisor.log`。要在有标签页的机器上走脚本驱动的 stdin 管道，
把 `session_command` 换回 rpc 形态即可（`pi --mode rpc --session-id {session}
--session-dir .pi/sessions -ns`）：代价是标签页里只剩 JSON 流，好处是宿主能用
stdin 推后续轮次。两条路的任务正文投递相同，argv 都不是正文的通道。

## 单独跑某个角色

工作区生成之后就是普通 Onlyne 工作区，模板脚本只负责把它们摆好：
...
`onlyne-board` 的 `serverRoots` 指向集群根（`~/.config/onlyne-sessions/config.json`），
刷新即得连接视图：标签页 ∩ 活着的会话，亮着的只有连着这个 workspace 的 pane。
