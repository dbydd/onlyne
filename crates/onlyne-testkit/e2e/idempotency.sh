...
# `causality.task` is a uuid v4 in the plan's envelope (docs/v1-PLAN.md line 152).
...
rows=$("$ONLYNE" --server-root "$tmp/server" ledger --role planner 2>/dev/null) || rows='{"ok":false,"data":{"ledger":[]}}'
printf '%s\n' "$rows" > "$tmp/rows.json"
count=$(json_field "$tmp/rows.json" '.data.ledger | length' 'len(json.load(sys.stdin).get("data",{}).get("ledger",[]))')
