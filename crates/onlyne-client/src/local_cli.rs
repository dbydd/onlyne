...
use onlyne_proto::{AdapterMsg, ClientOp, ControlArgs, Envelope, HelloArgs, HistoryArgs, LedgerQuery, MountKind, PluginOp, PROTOCOL_VERSION, QueryFaultsArgs, QueryRolesArgs, QuerySessionsArgs, ResBody, Subscribe};
...
pub const RESTART_HINT: &str = "onlyne: no running client; run `onlyne client restart` to mount this plugin";
...
pub fn valid_plugin_id(id: &str) -> bool {
    ...
    if bytes.is_empty() || bytes.len() > 32 { return false; }
    let first = bytes[0];
...
pub fn agent_install(workspace: &Path, package: &Path, plugin_id: &str, agent: Option<&str>) -> Result<Vec<String>> {
    if !valid_plugin_id(plugin_id) { return Err(anyhow!("onlyne: invalid plugin id {plugin_id}")); }
    let target = agent_package_dir(workspace, plugin_id);
    if target.exists() { return Err(anyhow!("onlyne: plugin {plugin_id} already installed")); }
    ...
    let manifest = format!("id = {plugin_id:?}\nbinary = {binary:?}\nagent = {agent_name:?}\ncapabilities = []\n");
...
pub fn agent_uninstall(workspace: &Path, plugin_id: &str) -> Result<Vec<String>> {
    if !valid_plugin_id(plugin_id) { return Err(anyhow!("onlyne: invalid plugin id {plugin_id}")); }
    let target = agent_package_dir(workspace, plugin_id);
    if !target.exists() { return Err(anyhow!("onlyne: no plugin {plugin_id} installed")); }
...
pub async fn install_verb(workspace: &Path, package: &Path, plugin_id: &str, agent: Option<&str>) -> Result<PluginAction> {
    let lines = agent_install(workspace, package, plugin_id, agent)?;
    let running = notify_client(workspace, plugin_id).await;
    Ok(PluginAction { lines, hint: (!running).then(|| RESTART_HINT.to_string()) })
}
...
pub fn plugin_exit_code(error: &anyhow::Error) -> i32 {
    if error.to_string().starts_with("onlyne: ") { 2 } else { 1 }
}
...
pub async fn notify_client(workspace: &Path, plugin_id: &str) -> bool {
    ...
    let Ok(stream) = UnixStream::connect(&socket).await else { return false };
    let io = AdapterIo::new(stream, CLIENT_PROBE_TIMEOUT, CLIENT_PROBE_TIMEOUT);
    let hello = HelloArgs { protocol: PROTOCOL_VERSION, plugin: format!("onlyne-client-cli:{plugin_id}"), version: env!("CARGO_PKG_VERSION").to_string(), kind: MountKind::Admin, capabilities: Vec::new(), mount: Some(onlyne_proto::Mount::Admin) };
...
fn install_package_files(package: &Path, target: &Path) -> Result<()> {
    if package.is_dir() {
        let entries: Vec<_> = std::fs::read_dir(package).with_context(|| format!("read {}", package.display()))?.collect::<std::result::Result<Vec<_>, _>>()?;
        for entry in &entries {
            let name = entry.file_name().to_string_lossy().into_owned();
            let source = entry.path();
            let destination = target.join(&name);
            if source.is_dir() { return Err(anyhow!("onlyne: package directory must be flat")); }
            std::fs::copy(&source, &destination).with_context(|| format!("copy {}", name))?;
        }
        return Ok(());
    }
    if package.extension().and_then(|ext| ext.to_str()) == Some("gz") {
        let file = std::fs::File::open(package).with_context(|| format!("open {}", package.display()))?;
        let decoder = flate2::read::GzDecoder::new(file);
        let mut archive = tar::Archive::new(decoder);
        archive.unpack(target).with_context(|| format!("unpack {}", package.display()))?;
        return Ok(());
    }
    Err(anyhow!("onlyne: package must be a directory or a .tar.gz file"))
}
...
fn find_plugin_binary(target: &Path) -> Result<String> {
    ...
    for entry in std::fs::read_dir(target).with_context(|| format!("read {}", target.display()))? {
        let entry = entry?;
        let name = entry.file_name().to_string_lossy().into_owned();
        if name.starts_with("onlyne-agent-") && entry.file_type()?.is_file() { binaries.push(name); }
    }
    binaries.sort();
    if binaries.len() == 1 { Ok(binaries.remove(0)) } else { Err(anyhow!("onlyne: package must contain exactly one onlyne-agent-* executable")) }
}

#[cfg(unix)]
fn set_executable(path: &Path) -> Result<()> {
    use std::os::unix::fs::PermissionsExt;
    let mut permissions = std::fs::metadata(path).with_context(|| format!("stat {}", path.display()))?.permissions();
    permissions.set_mode(0o755);
    std::fs::set_permissions(path, permissions).with_context(|| format!("chmod {}", path.display()))?;
    Ok(())
}
...
fn remove_plugin_entry(workspace: &Path, plugin_id: &str) -> Result<()> {
    let config = RoleWorkspace::resolve(workspace).config_path();
    let text = std::fs::read_to_string(&config).with_context(|| format!("read {}", config.display()))?;
    ...
    while let Some(line) = lines.next() {
        if line.trim() == "[[plugin]]" {
            ...
            let owns = block.iter().any(|entry| entry.trim() == format!("id = {plugin_id:?}"));
            if owns { continue; }
            for entry in block { output.push_str(&entry); output.push('\n'); }
        } else {
...
mod agent_tests {
    ...
    fn agent_install_and_uninstall_round_trip() {
        ...
        std::fs::write(package.path().join("onlyne-agent-demo"), "#!/bin/sh\nexit 0\n").unwrap();
        let lines = agent_install(workspace.path(), package.path(), "demo", Some("demo-agent")).unwrap();
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            let mode = std::fs::metadata(target.join("onlyne-agent-demo")).unwrap().permissions().mode() & 0o777;
    ...
    fn agent_missing_id_reports_not_installed() {
        ...
        assert_eq!(result.unwrap_err().to_string(), "onlyne: no plugin ghost installed");
    }
