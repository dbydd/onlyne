...
| `.onlyne/cache/pi-panes/<pane_key, ':' → '-'>.json` | | the Orca pane one pi process runs in: one file per pane, published by the pi adapter (`integrations/pi-onlyne`); appears when pi mounts, rewritten per task, removed when its session ends |
| `.onlyne/cache/pi-pane.json` | | the pre-v1 single-file claim layout: still read by the supervisor board through the transition, never written |
