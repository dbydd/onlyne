...
use rustls::{ClientConfig, DigitallySignedStruct, Error as TlsError, RootCertStore, ServerConfig, SignatureScheme};
...
impl fmt::Display for PinMismatchMarker {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "certificate pin mismatch: expected {}, got {}", self.expected, self.got)
    }
}
...
pub fn gen_self_signed(common_name: &str, validity_days: i64) -> Result<ServerCert, NetError> {
    if common_name.is_empty() {
        return Err(NetError::Crypto("common name must not be empty".to_string()));
    }
    if validity_days <= 0 {
        return Err(NetError::Crypto("validity_days must be positive".to_string()));
    }
    ...
    let cert = params.self_signed(&key_pair).map_err(|error| NetError::Crypto(error.to_string()))?;
    let cert_pem = cert.pem().into_bytes();
    let key_pem = key_pair.serialize_pem().into_bytes();
    let spki_pin = spki_pin_of(cert.der().as_ref())?;
    Ok(ServerCert { key_pem, cert_pem, spki_pin })
}
...
fn load_pair(path: &Path) -> Result<ServerCert, NetError> {
    ...
    let cert = items.iter().find_map(|item| match item {
    ...
    let (_, parsed) = parse_x509_certificate(&cert).map_err(|error| NetError::Crypto(error.to_string()))?;
    if !parsed.validity().is_valid() {
        return Err(NetError::Crypto("stored certificate is expired or not yet valid".to_string()));
    }
    Ok(ServerCert {
...
fn extract_first_key_pem(bytes: &[u8]) -> Result<Vec<u8>, NetError> {
    for marker in [b"-----BEGIN PRIVATE KEY-----".as_slice(), b"-----BEGIN RSA PRIVATE KEY-----", b"-----BEGIN EC PRIVATE KEY-----"] {
        if let Some(start) = bytes.windows(marker.len()).position(|window| window == marker) {
            if let Some(end_rel) = bytes[start..].windows(b"-----END ".len()).position(|window| window == b"-----END ") {
                let end_start = start + end_rel;
                let end = bytes[end_start..].iter().position(|byte| *byte == b'\n').map_or(bytes.len(), |offset| end_start + offset + 1);
...
fn extract_first_cert_pem(bytes: &[u8]) -> Result<Vec<u8>, NetError> {
    ...
    let end_rel = bytes[start..].windows(end_marker.len()).position(|window| window == end_marker)
        .ok_or_else(|| NetError::Crypto("certificate PEM is malformed".to_string()))?;
    let end_start = start + end_rel;
    let end = bytes[end_start..].iter().position(|byte| *byte == b'\n').map_or(bytes.len(), |offset| end_start + offset + 1);
...
pub fn server_config(cert: &ServerCert) -> Result<ServerConfig, NetError> {
    ...
    let key = rustls_pemfile::private_key(&mut key_reader)?.ok_or_else(|| NetError::Crypto("private key PEM is missing".to_string()))?;
...
pub fn client_config(expected_pin: &str) -> Result<ClientConfig, NetError> {
    if !expected_pin.starts_with("sha256/") {
        return Err(NetError::MalformedKey("expected certificate pin must use sha256/ prefix".to_string()));
    }
    let provider = Arc::new(rustls::crypto::ring::default_provider());
    let verifier = Arc::new(PinnedVerifier { expected: expected_pin.to_string(), provider: provider.clone() });
...
impl ServerCertVerifier for PinnedVerifier {
    fn verify_server_cert(
        ...
        let got = spki_pin_of(end_entity.as_ref()).map_err(|_| TlsError::InvalidCertificate(rustls::CertificateError::BadEncoding))?;
        if got != self.expected {
            return Err(TlsError::Other(rustls::OtherError(Arc::new(PinMismatchMarker {
                expected: self.expected.clone(),
                got,
            }))));
        }
        let mut roots = RootCertStore::empty();
        roots.add(end_entity.clone()).map_err(|_| TlsError::InvalidCertificate(rustls::CertificateError::BadEncoding))?;
        let verifier = rustls::client::WebPkiServerVerifier::builder_with_provider(Arc::new(roots), self.provider.clone())
            .build()
    ...
    fn verify_tls13_signature(&self, message: &[u8], cert: &CertificateDer<'_>, dss: &DigitallySignedStruct) -> Result<HandshakeSignatureValid, TlsError> {
        let mut roots = RootCertStore::empty();
        roots.add(cert.clone()).map_err(|_| TlsError::InvalidCertificate(rustls::CertificateError::BadEncoding))?;
        let verifier = rustls::client::WebPkiServerVerifier::builder_with_provider(Arc::new(roots), self.provider.clone())
            .build()
            .map_err(|error| TlsError::General(error.to_string()))?;
...
impl<T> Pipe for T {
    fn pipe<U>(self, f: impl FnOnce(Self) -> U) -> U { f(self) }
}
