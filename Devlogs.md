...
- The npm package `pi-onlyne` is version 1.2.2, published 2026-09-26. All nineteen crates reached crates.io at 1.4.1 the same day, in the dependency order `scripts/publish.py` records, each through `cargo publish --locked -p onlyne-<crate>` at tag `v1.4.1` with Cargo's packaging sandbox build. The link to crates.io carried its slow periods: four crates (`onlyne-net`, `onlyne-session`, `onlyne-client`, `onlyne-gateway`) took a second attempt, and `onlyne-gateway-telegram` uploaded on its second attempt while the availability wait failed, so its third attempt answered "already exists" and the script counted that as success. The registry API reads 1.4.1 as each crate's max version with nothing yanked.
...
- npm package `pi-onlyne` 版本为 1.2.2，于 2026-09-26 发布。同日十九个 crate 全部以 1.4.1 到达 crates.io，顺序即 `scripts/publish.py` 记录的依赖顺序，每个都在 tag `v1.4.1` 上以 `cargo publish --locked -p onlyne-<crate>` 上传并经过 Cargo 的 packaging sandbox build。crates.io 链路当时偏慢：`onlyne-net`、`onlyne-session`、`onlyne-client`、`onlyne-gateway` 各自在第二次尝试才成功；`onlyne-gateway-telegram` 的第二次尝试上传完成但等待可用性失败，第三次尝试由 "already exists" 回答，脚本将其计为成功。registry API 显示每个 crate 的 max version 均为 1.4.1，且无 yanked。
